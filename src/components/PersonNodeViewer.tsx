...
const PersonNodeViewer: React.FC<PersonNodeProps> = memo(({ data, id }) => {
  const { 
    removePerson, 
    setSelectedNode, 
    selectedNodeId, 
      ...
      <div className="absolute top-2 right-2 z-10">
        <DropdownMenu>
          ...
          <DropdownMenuContent className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
            <DropdownMenuItem onClick={() => setNodeViewMode('normal')}>
              <User size={14} className="mr-2" />
              Normal View
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => setNodeViewMode('fullImage')}>
    ...
    >
      ...
      <div className="p-4 flex flex-col items-center text-center relative">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                onClick={handleActionClick}
              >
                <View size={14} className="text-gray-500" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
              <DropdownMenuItem onClick={() => setNodeViewMode('normal')}>
        ...
        <div className="relative mb-3">
          <div className={cn("absolute -inset-1 rounded-full", statusRingColor())} />
          {data.image ? (
            <img
              src={data.image}
              alt={data.name}
    ...
    <TooltipProvider delayDuration={300}>
      <div className="relative group" onClick={handleNodeClick}>
        ...
        <Handle
          ...
          className="!bg-transparent !w-4 !h-4 !border-2 !border-gray-400/50 hover:!border-sky-500 !transition-colors !duration-300"
        />
        {nodeViewMode === 'fullImage' ? renderFullImageView() : renderNormalView()}
      </div>
    </TooltipProvider>
  );
});
